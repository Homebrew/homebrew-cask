cask "clickshare" do
  ...
  url "https://www.barco.com/services/website/en/TdeFiles/Download?FileNumber=R3306192&TdeType=3&MajorVersion=#{format("%02d", version.major)}&MinorVersion=#{format("%02d", version.minor)}&PatchVersion=#{format("%02d", version.patch)}&BuildVersion=#{format("%03d", version.split(".").last)}&ShowDownloadPage=False"
  name "ClickShare"
  ...
  livecheck do
    ...
    strategy :page_match
    regex(/ClickShare Desktop App Mac v(\d+.\d+.\d+.\d+)/)
  end
